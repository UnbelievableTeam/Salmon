...
public class SliderMenu : MonoBehaviour {
	...
	void Start () {
		...
		ScrollContent.GetComponent<RectTransform>().sizeDelta=new Vector2((LevelThumbnails.Count+2)*(Element_Width+2*Element_Margin),Element_Height+40*Element_Margin);
		//-------------------------------------------------------------------------------------------
	...
	void Update () {
		...
		if (DesktopPlatform == false) {
			if (Input.touchCount == 0) {
				for (int s=0; s<LevelThumbnails.Count; s++) {
					if (VerticalScrollBar.GetComponent<Scrollbar> ().value > (ScrollSteps / 2) + (s - 1) * (ScrollSteps) && VerticalScrollBar.GetComponent<Scrollbar> ().value <= Mathf.Clamp (ScrollSteps / 2 + s * ScrollSteps, 0, 1)) {
						VerticalScrollBar.GetComponent<Scrollbar> ().value = Mathf.Lerp (VerticalScrollBar.GetComponent<Scrollbar> ().value, s * ScrollSteps, 0.1f);
					}
				}
			}
		} 
		...
		if (ButtonClicked == true) {
			VerticalScrollBar.GetComponent<Scrollbar> ().value=Mathf.Lerp(VerticalScrollBar.GetComponent<Scrollbar> ().value,k,0.1f);
		}
		//-------------------------------------------------------------------------------------------
	}
	...
	public void ContentDrag(){
		ButtonClicked = false;
		k=Mathf.Clamp (VerticalScrollBar.GetComponent<Scrollbar> ().value,0,1);
